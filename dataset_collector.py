...
class DatasetCollector:
    ...
    def process_video(self, video_path, save_path):
        ...
        np.save(save_path, sequences_np)
    ...
    def process_folder(self, folder_path, label):
        ...
        # 저장 폴더 생성
        save_dir = os.path.join('data', 'datasets', label)
        os.makedirs(save_dir, exist_ok=True)

        print(f"\n[{label}] {folder_path} ({len(video_files)}개 영상)")
        for i, filename in enumerate(video_files, start=1):
            video_path = os.path.join(folder_path, filename)
            save_path  = os.path.join(save_dir, f"dataset_{label}_{i}.npy")
